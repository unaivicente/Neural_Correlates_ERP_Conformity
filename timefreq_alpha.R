...
modelalpha180 <- brm(timefreq8_12_180 ~ TrialRep + (1|ID/AbsSuj) + (1+TrialRep||electrodo), family=Gamma(link="log"), data=df,
...
modelalpha230 <- brm(timefreq8_12_230 ~ TrialRep + (1|ID/AbsSuj) + (1+TrialRep||electrodo), family="skew_normal", data=df,
                     prior=prior, chains=4, cores=4, iter=10000, warmup=1000,
                     inits='0', control=list(adapt_delta=0.90, max_treedepth = 10),
                     silent=F, refresh=0, open_progress=F)


saveRDS(modelalpha180, file = "modelalpha180.rds")
saveRDS(modelalpha230, file = "modelalpha230.rds")


set_pushover_user(user = "u2yje7zfuhi3rzs4zwpkrostp6wrdj")
set_pushover_app(token = "a36jriy3tey1gddfh1yzetjjy2bhxy")

pushover(message="Processing for TimeFreqs ALPHA is ready, go fetch it!")
