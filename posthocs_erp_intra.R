...
intra <- readRDS('intradif_v1.rds') #lead as intercept

posterior_data <- as_draws(intra)
...
for (i in 1:nrow(df)){
  ...
  b1 <- posterior_data[['b_intra_scale']] + posterior_data[[sprintf('r_electrode.C_%s_%s.intra_scale.',df$a[i],df$b[i])]]
  ...
  b4 <- posterior_data[['b_intra_scale.TrialRep1']]+posterior_data[[sprintf('r_electrode.C_%s_%s.intra_scale.TrialRep1.',df$a[i],df$b[i])]]
  b5 <- posterior_data[['b_intra_scale.TrialRep2']]+posterior_data[[sprintf('r_electrode.C_%s_%s.intra_scale.TrialRep2.',df$a[i],df$b[i])]]
  ...
  hyp1test$Hypothesys <- "Intra: TrialRep 1 over Intercept"
  hyp1test$ElecTime <- sprintf('%s_%s',df$a[i],df$b[i])
  hyp2test$Hypothesys <- "Intra: TrialRep 2 over Intercept"
  hyp2test$ElecTime <- sprintf('%s_%s',df$a[i],df$b[i])
  hyp3test$Hypothesys <- "Intra: Difference 2 over 1"
